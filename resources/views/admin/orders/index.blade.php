...
<div class="py-4">
    ...
    <div class="filters-row mb-3">
        <input type="text" id="searchInput" name="search" data-filter="search" placeholder=" رقم الطلب،الهاتف أو البريد إلكتروني..." class="form-control search-filter-input" style="max-width: 280px;" value="{{ request('search', '') }}">
        <select id="statusFilter" name="status" data-filter="status" class="form-select search-filter-input" style="max-width: 150px;">
            <option value="" {{ request('status', '') === '' ? 'selected' : '' }}>جميع الحالات</option>
            <option value="pending" {{ request('status', '') === 'pending' ? 'selected' : '' }}> قيد الانتظار </option>
            <option value="proessing" {{ request('status', '') === 'proessing' ? 'selected' : '' }}> قيد المعالجة</option>
                ...
                <tr>
                    ...
                    <td>
                        <div class="d-flex gap-2 justify-content-end">
                            <a href="{{ route('admin.orders.edit', $order->id) }}" class="btn btn-details btn-sm">التفاصيل</a>
                            <a href="{{ route('admin.orders.edit', $order->id) }}" class="btn btn-primary btn-sm">تعديل/تحديث</a>
                        </div>
                    </td>
                </tr>
                @endforeach
